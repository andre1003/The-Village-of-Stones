var stage4State = { // Objeto da Fase 4
	addMusicaSons: function() { // Adiciona a música e os sons no jogo
		// Música e sons
		this.musica_fase_4_loop = game.add.audio('musica_fase4_batalha');
		this.musica_fase_4_loop.loop = true;
		this.musica_fase_4_loop.volume = .4;
		this.musica_fase_4_loop.play();
		...
		this.som_hit_basico_heroi = game.add.audio('som_hit_basico');
		this.som_hit_basico_heroi.volume = .2;
		this.som_hit_magico_heroi = game.add.audio('som_hit_magico');
		this.som_hit_magico_heroi.volume = .2;
		this.som_hit_basico_monstro = game.add.audio('som_hit_magico');
		this.som_hit_basico_monstro.volume = .2;
		this.som_hit_magico_monstro = game.add.audio('som_trovao');
		this.som_hit_magico_monstro.volume = .3;
	...
	addObjetos: function() { // Adiciona os objetos na tela do jogo
		...
		// Ataque básico do monstro
		this.img_ataque_basico = game.add.sprite(715,game.world.height - 330,'ataque_magico_2');
		this.img_ataque_basico.smoothed = false;
		this.img_ataque_basico.scale.setTo(3,3);
		this.img_ataque_basico.tint = 0xffe741;
		this.img_ataque_basico.visible = false;

		// Ataque maǵico do monstro
		this.img_ataque_magico_2 = game.add.sprite(0,0,'trovao');
		this.img_ataque_magico_2.smoothed = false;
		this.img_ataque_magico_2.scale.setTo(5,5);
		...
		// Opções - Botão Pedra
		this.barra_pedra = game.add.sprite(game.world.centerX + 150, 320, 'barra_pedra_3');
		this.barra_pedra.smoothed = false;
		this.barra_pedra.scale.setTo(4,4);

		this.txt_ar = game.add.text(game.world.centerX + 205, 338, 'AR', {font: "20px pixel_arial_r", fill: "#ffffff", align: "center" });
		this.txt_agua = game.add.text(game.world.centerX + 189, 402, 'AGUA', {font: "20px pixel_arial_r", fill: "#808080", align: "center" });
	...
	opcaoMonstro: function() {
		...
		if(opcao_ataque == 0) {
			this.dano_monstro_aux = definicaoAtaqueFinal('basico','voss',this.turno + 1);
			this.ataque_basico_monstro = true;
			this.ataque_magico_monstro = false;
		}
		else if(opcao_ataque == 1) {
			this.dano_monstro_aux = definicaoAtaqueFinal('magico','voss',this.turno + 1);
			this.ataque_basico_monstro = false;
	...
	ataqueHeroi: function() {
		if(this.ataque_basico_heroi == true && this.dano_heroi > 0) 
			this.som_hit_basico_heroi.play();
		else if(this.ataque_magico_heroi == true && this.dano_heroi > 0)
			this.som_hit_magico_heroi.play();
		else
			this.som_errou.play();
	...
	ataqueMonstro: function() {
		if(this.ataque_basico_monstro == true && this.dano_monstro > 0) 
			this.som_hit_basico_monstro.play();
		else if(this.ataque_magico_monstro == true && this.dano_monstro > 0) 
			this.som_hit_magico_monstro.play();
		else
			this.som_errou.play();
		...
		if(this.ataque_magico_monstro == true) {
			...
			this.txt_ataque_magico_2.visible = true;
			this.img_ataque_magico_2.animations.play('stop');
		}

		if(this.ataque_basico_monstro == true) {
			this.img_ataque_basico.visible = true;
			game.add.tween(this.img_ataque_basico).to({x: '-520'}, 1000, Phaser.Easing.Linear.None, true);
		}
