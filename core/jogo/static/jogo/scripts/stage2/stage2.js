var stage2State = { // Objeto da Fase 2
	addMusicaSons: function() { // Adiciona a música e os sons no jogo
		// Música e sons
		this.musica_fase_2_loop = game.add.audio('musica_fase2_batalha');
		this.musica_fase_2_loop.loop = true;
		this.musica_fase_2_loop.volume = .4;
		this.musica_fase_2_loop.play();
	...
	addObjetos: function() { // Adiciona os objetos na tela do jogo
		...
		this.img_ataque_basico = game.add.sprite(750,game.world.height - 335,'folha_zayin');
		this.img_ataque_basico.smoothed = false;
		this.img_ataque_basico.scale.setTo(4,4);
	...
	update: function(){	
		...
		if(this.executar == true) {
			...
			enviarDados(
				2, this.turno+1, this.vida_heroi  - this.dano_monstro,
	...
	opcaoMonstro: function() {
		...
		if(this.opcao_ataque_monstro == 0) {
			this.dano_monstro_aux = definicaoAtaqueFinal('basico','zayin',this.turno + 1);
			this.ataque_basico_monstro = true;
			this.ataque_magico_monstro = false;
		}
		else if(this.opcao_ataque_monstro == 1) {
			this.dano_monstro_aux = definicaoAtaqueFinal('magico','zayin',this.turno + 1);
			this.ataque_basico_monstro = false;
	...
	ataqueMonstro: function() {
		...
		if(this.ataque_basico_monstro == true) {
			...
			game.add.tween(this.img_ataque_basico).to({x: '-560'}, 1000, Phaser.Easing.Linear.None, true);
		}
		else if(this.ataque_magico_monstro == true) {
			this.img_ataque_magico_2.visible = true;
			game.add.tween(this.img_ataque_magico_2).to({x: '-560'}, 1000, Phaser.Easing.Linear.None, true);
		}
	...
	estadoInicial: function() {
		...
		if(this.vida_monstro <= 0) {
			...
			game.global.fase_concluida = 2;

			game.time.events.add(540, function() {
				this.musica_fase_2_loop.stop();
            	game.state.start('end_stage2');
			}, this);
		}
